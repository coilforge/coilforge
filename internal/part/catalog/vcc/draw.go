...
// File overview:
// draw renders vcc geometry and anchors in world space for this part.
// Subsystem: part catalog (vcc) drawing.
...
func (self *Vcc) layoutName() string {
	slots := RotationSlots
	if slots <= 0 {
		slots = 4
	}
...
func (self *Vcc) Bounds() core.Rect {
...
func (self *Vcc) Anchors() []core.PinAnchor {
	return part.AnchorsFromVectorMarkerIDs(self.layoutName(), self.drawBase(), vccPinMarkerMap(self))
}

// HitTest handles hit test.
func (self *Vcc) HitTest(pt core.Pt) part.HitResult {
...
// Draw draws its work.
func (self *Vcc) Draw(ctx part.DrawContext) {
